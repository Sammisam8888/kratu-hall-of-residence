    ...
    <PageTransition>
      <div ref={containerRef} className="pb-32">
        ...
        <section className="relative min-h-[60vh] md:min-h-[80vh] flex flex-col items-center justify-center pt-32 px-4 overflow-hidden">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[300px] bg-yellow-100/5 rounded-[100%] blur-[100px] -z-10" />
            ...
            {profiles.map((profile, i) => (
              <motion.div
                ...
                className={`glass-card group overflow-hidden flex flex-col ${
                  i === 0 ? "md:col-span-2 lg:col-span-3 md:flex-row" : "h-full"
                }`}
              >
                {/* Image Section */}
                <div className={`relative bg-dark-900 overflow-hidden shrink-0 border-b md:border-b-0 md:border-r border-white/5 ${
                  i === 0 ? "w-full md:w-1/3 aspect-[4/3] md:aspect-[3/4] lg:aspect-auto lg:h-[450px]" : "w-full aspect-[3/4]"
                }`}>
                  {profile.placeholder ? (
                ...
                <div className={`relative flex flex-col p-8 lg:p-10 flex-1 justify-center`}>
                  ...
                  <div className="space-y-4 mt-auto">
                    {profile.email && (
                      <div className="flex items-center gap-4 text-white/50 text-sm bg-white/5 px-4 py-3 rounded-xl border border-white/5">
                        <span className="text-yellow-100 font-bold uppercase tracking-widest text-[10px]">Email</span>
                        <span className="font-mono text-xs break-all">{profile.email}</span>
                      </div>
                    )}
                    {profile.phone && (
                      <div className="flex items-center gap-4 text-white/50 text-sm bg-white/5 px-4 py-3 rounded-xl border border-white/5">
                        <span className="text-yellow-100 font-bold uppercase tracking-widest text-[10px]">Tel</span>
